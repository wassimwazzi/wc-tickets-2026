# High Priority
- [ ] Offer flow:
    - [ ] When buyer submits offer, create pending offer and notify seller
    - [ ] Seller can accept or decline offer; if accepted, allow seller and buyer to chat (later create escrow transaction) and notify buyer
    - [ ] Buyer can withdraw offer if it's still pending
    - [ ] Buyer should be able to choose how many tickets in their offer (currently hardcoded to 1)
    - [ ] Seller can choose how many tickets can be sold (for example don't leave me with 1 ticket if I have 4 to sell)
    - [ ] Don't allow buyer to submit multiple offers on the same listing unless they withdraw the previous one
    - [ ] Once offer is accepted, deduct the sold quantity from the listing and update status to pending if there are still tickets available, or sold if all tickets are sold, and remove it from the browse page
